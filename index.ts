...
client.on('message', async (message) => {
    ...
    if (message.channel.type === "dm") {
        ...
        if (content.toLocaleLowerCase() === 'yes')
        {
            ...
            return;
        } else if (content.toLocaleLowerCase() === 'no') {
            const guildMember = server.member(message.author);
            ...
            if (splitMessage.length !== 2 || !splitMessage[0]?.startsWith('!') || !splitMessage[1].length) {
                await message.channel.send("Looks like there was a problem with your message. Make sure it is two words, the first being !ign followed by you Oldschool RuneScape name")
                return;
            }
            if (splitMessage[0].toLocaleLowerCase() === '!ign') {
                ...
                if (server) {
                    try {
                        await server.member(message.author)?.setNickname(splitMessage[1]);
...
client.on('guildMemberAdd', async (member) => {
    ...
    if (welcomeChannel && welcomeChannel.isText()) {
        try {
            await welcomeChannel.send(`Welcome <@${member.id}>! Keep an eye out for a PM to complete your registration!`);
        } catch (e) {
